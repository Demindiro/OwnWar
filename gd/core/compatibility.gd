...
static func convert_vehicle_data(data: Dictionary):
	...
	converted_data["game_version"] = Util.version_vector_to_str(game_version)
...
static func convert_game_data(data: Dictionary):# -> Dictionary:
	...
	if data_version > load("res://core/ownwar.gd").VERSION:
...
static func add_block_mapping(from: String, to: String, version: Vector3) -> void:
	assert(version <= load("res://core/ownwar.gd").VERSION)
