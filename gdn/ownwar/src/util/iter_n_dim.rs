...
pub fn iter_2d<T>(start: (T, T), end: (T, T)) -> impl Iterator<Item = (T, T)>
where
	T: Copy + Step + 'static,
{
...
pub fn iter_3d<T>(start: (T, T, T), end: (T, T, T)) -> impl Iterator<Item = (T, T, T)>
where
	T: Copy + Step + 'static,
{
	iter_2d((start.0, start.1), (end.0, end.1))
